...
func TestFilteringOfARegistry(t *testing.T) {
	...
	Convey("Given a simple but real registry", t, func() {
		...
		Convey("We can push images and retrieve", func() {
			...
			imageHandler, err := NewDockerCLIHandler(DockerHubRegistry, regInfo)
			So(err, ShouldBeNil)
			err = imageHandler.PullTagPush("alpine", "", "stable")
			So(err, ShouldBeNil)
			err = imageHandler.PullTagPush("alpine", "mynamespace", "0.1")
			So(err, ShouldBeNil)
			err = imageHandler.PullTagPush("alpine", "", "0.1")
			So(err, ShouldBeNil)
			err = imageHandler.PullTagPush("busybox", "mynamespace", "0.1-stable")
			So(err, ShouldBeNil)
